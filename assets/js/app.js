...
const handleCollapseSummary = function () {
    if ($('.btnCollapseContent').length) {
        $('.btnCollapseContent').click(function () {
            let target = $(this).attr('data-id');

            if ($(target).hasClass('is-collapsing')) {
                $(target).removeClass('is-collapsing');
                $(this).html('Thu gọn <i class="fal fa-angle-up ms-2"></i>');
            } else {
                $(target).addClass('is-collapsing');
                $(this).html('Hiện thêm <i class="fal fa-angle-down ms-2"></i>');
            }
        });
    }
}

const handleCallPopupStudy = function () {
    let popupStudy = $("#popupStudy");
    if (popupStudy.length) {
        $('.callPopupStudy').click(function () {
            popupStudy.modal('show');
            handleVideoStudy();
        })
    }
}

const handleVideoStudy = function () {
    let options = {
        playbackRates: [0.5, 0.75, 0.85, 1, 1.25, 1.5, 2],
        autoplay: 'true',
        controls: true,
        preload: 'auto',
        plugins: {
    ...
    let pathVideo = [
        ...
        {
            src: "https://stream.unica.vn/unica/_definst_/unica/129/12840/720.mp4/playlist.m3u8?wowzatokenendTime=1710857355&wowzatokenstartTime=1710850155&wowzatokenhash=Yu99l733z00dI_UbVHe4RbQidZU-eX2Ef2hdmVtP2hM=&uid=719359&expired=1710850303&token=9e36e28d1d406f9610941e8d169037f58496f41c",
            type: "video/mp4",
            label: "720",
            res: "720"
        }
    ...
    let videoStudy = videojs('studyVideo', options, function () {
        let player = this;
    ...
    $('.study-body .quality').hide();
    b = true;
    $('.study-body .vjs-spacer').click(function () {
        if (b) {
            $('.study-body .quality').show();
            b = false;
        } else {
            $('.study-body .quality').hide();
            b = true;
    ...
    $('.study-body .quality .changeQuality').click(function () {
        if($(this).hasClass('active')) {
            return false;
        }
        $('.study-body .quality .changeQuality i').hide();
        $('.study-body .quality .changeQuality').removeClass('active');
        $('.study-body .quality').hide();
        b = true;
        $(this).find('i').show();
        ...
        switch (quality) {
            case '360':
                ...
                console.log(videoStudy)
                videoStudy.play();
                break;
            case '720':
                videoStudy.src(
                    {
                        src: "./assets/images/video-test.mp4",
                        type: "video/mp4",
                        label: "720",
                        res: "720"
                    }
